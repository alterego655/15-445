...
class BufferPoolManager {
 ...
 protected:
  ...
  /**
   * Get the target page from the buffer pool
   * @param the frame_id of the page
   * @return the target page
   */
  inline Page *GetTargetPage(frame_id_t frame_id_offset) {
    assert(frame_id_offset != INVALID_FRAME_ID);
    assert(pool_size_ > size_t(frame_id_offset));
    Page *page = pages_ + frame_id_offset;
    return page;
  }

  /**
   * Get the frame_id of buffer pool
   * @param the page_id of the current page
   * @return the frame_id of the requested page
   */
  inline frame_id_t seekFrame(page_id_t page_id) {
  ...
  /**
   * get a replacement page from the free list or the replacer; free list always first.
   * @return the replacement page
   */
  frame_id_t GetReplaceblePage();
