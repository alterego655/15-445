...
std::vector<Value> IndexScanExecutor::GetValFromTuple(const Tuple &tuple) {
  std::vector<Value> res;
  for (auto &col : GetOutputSchema()->GetColumns()) {
    Value val = tuple.GetValue(GetOutputSchema(), GetOutputSchema()->GetColIdx(col.GetName()));
    res.push_back(val);
  }
  return res;
}
...
bool IndexScanExecutor::Next(Tuple *tuple, RID *rid) {
  ...
  *rid = (*idx_itr).second;
  ...
  std::vector<Value> val = GetValFromTuple(*tuple);
