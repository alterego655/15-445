 ...
 */
INDEX_TEMPLATE_ARGUMENTS page_id_t B_PLUS_TREE_LEAF_PAGE_TYPE::GetNextPageId() const { return next_page_id_; }
...
/**
 * Helper method to find the index i so that array[i].first == key
 */

INDEX_TEMPLATE_ARGUMENTS
int B_PLUS_TREE_LEAF_PAGE_TYPE::FindKey(const KeyType &key, const KeyComparator &comparator) const {
  int begin = 0;
  int end = GetSize() - 1;
  while (begin <= end) {
    int mid = begin + (end - begin) / 2;
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveLastToFrontOf(BPlusTreeLeafPage *recipient) {
  assert(GetSize() > 0);
  int page_size = GetSize();
  recipient->CopyFirstFrom(GetItem(page_size - 1));
  SetSize(page_size - 1);
}
/*
