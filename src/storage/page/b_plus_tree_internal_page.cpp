...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::MoveHalfTo(BPlusTreeInternalPage *recipient,
  ...
  int size = GetSize();
  int sum = GetMaxSize() + 1;
  assert(size == sum);
  int move_size = size - sum / 2;
  int start = size - move_size;
  recipient->CopyNFrom(array + start, sum - move_size, buffer_pool_manager);
  SetSize(start);
}
 ...
 */
INDEX_TEMPLATE_ARGUMENTS void B_PLUS_TREE_INTERNAL_PAGE_TYPE::Remove(int index) {
  assert(0 <= index && index < GetSize());
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::CopyFirstFrom(const MappingType &pair, BufferPoolManager *buffer_pool_manager) {
  ...
  Page *page = buffer_pool_manager->FetchPage(child_page_id);
  assert(page != nullptr);
  BPlusTreePage *child_page = reinterpret_cast<BPlusTreeInternalPage *>(page->GetData());
  child_page->SetParentPageId(GetPageId());
